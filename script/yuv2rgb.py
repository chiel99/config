...
parser.add_argument('--width', default=4032, type=int)
parser.add_argument('--height', default=3024, type=int)
...
class NV21Converter(Converter):
	...
	def Convert(self):
		...
		if "Main" in self.filename:
			folder = "./left/"
		else:
			folder = "./right/"
		converted_image_filename = folder + self.filename.split('.')[1] + ".jpg"
		...
		size_of_frame = int(((3.0/2.0)*self.height*self.width))
		...
		converted_image.save(converted_image_filename, "JPEG")
		print("Saved " + converted_image_filename)
...
def main():
	...
	converter = NV21Converter(args.filename, args.width, args.height, args.stride, args.frame)
	converter.Convert()
